...
If that fails due to package dependencies or the wrong platform, try to remove the `uv.lock` file.

## Run Experiments

The main script is `run_mt.py`. It has a few configuration parameters, with the most important being `--setting` to pick the dataset to run (`short`: SDD shortterm prediction, `long`: SDD longterm prediction, `ind`: inD longterm prediction).

Note: To be able to run the inD experiments, you need to first [request access to the inD dataset](https://levelxdata.com/ind-dataset/) and then preprocess it as described in the Y-net README below.

To run all experiments, then run the following commands:
```
python run_mt.py --setting short
python run_mt.py --setting long
python run_mt.py --setting ind
```
This will take approximately one hour per experiment on a normal desktop computer with a single CUDA-capable GPU.

Result files: Running the experiments will create three files (all already contained in the repository): `results_sdd_trajnet_full.csv`, `results_sdd_longterm_full.csv`, and `results_ind_longterm_full.csv`.

To evaluate the experiments and reproduce tables and figures, you can then run the notebook `mt_evaluation.py`.
This will produce three files:

- `results_trajectory.tex` (Table 3 from the paper)
