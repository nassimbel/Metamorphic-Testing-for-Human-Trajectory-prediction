...
from test_ynet import torch_multivariate_gaussian_heatmap
    ...
    with torch.no_grad():
        ...
        for trajectory, meta, scene in val_loader:
            ...
            for i in range(0, len(trajectory), batch_size):
                ...
                input_trajectory = trajectory[i : i + batch_size, :obs_len, :]

                # assert input_trajectory.shape == (1, obs_len, 2)

                pred_traj, future_samples, waypoint_samples, _ = predict(
                    model=model,
                ...
                val_FDE.append(
                    (
                        (((gt_goal - waypoint_samples[:, :, -1:]) / resize) ** 2).sum(
                            dim=3
                        )
                        ** 0.5
                    ).min(dim=0)[0]
...
def predict(
    ...
    if scene_image.shape[1] == 3:
        # Segmentation not yet performed
        scene_image = model.segmentation(scene_image)

    # Create Heatmaps for past and ground-truth future trajectories
    _, _, H, W = scene_image.shape
    observed = input_trajectory.reshape(-1, 2).cpu().numpy()
    ...
    if use_CWS and len(waypoints) > 1:
        ...
        last_observed = input_trajectory[:, -1].to(
            device
        )  # trajectory[i : i + batch_size, obs_len - 1].to(device)  # [N, 2]
            ...
            for waypoint_num in reversed(range(len(waypoints) - 1)):
                ...
                if g_num // num_goals == 0:
                    ...
                    waypoint_samples = sampling(
                        waypoint_map.unsqueeze(1), num_samples=1, rel_threshold=0.05
                    )
    ...
    return pred_traj, future_samples, waypoint_samples, pred_waypoint_map_sigmoid
